...
public class NewGetLatestPostServiceRF {
    private boolean bExecuted = false;
    IGetLatestPost service = null;
    ...
    public NewGetLatestPostServiceRF() {
        ...
        service = retrofit.create(IGetLatestPost.class);
    ...
    public void getLatestPost() {
        ...
        Call<GetCategoryPost> repoMedia = service.GetLatestPost();
        Log.d("url", "url(getNewPaginatedPosts)=>" +service.GetLatestPost().request().url().toString());
        bExecuted = false;
                    ...
                    if (posts != null) {
                        ...
                        for (int i = 0; i < posts.length(); i++) {
                            try {
                                ...
                                JSONArray categories = eachPost.getJSONArray("categories");
                                if (categories.length() != 0) {
                                    JSONObject firstCategorie = categories.getJSONObject(0);


                                    if (firstCategorie != null) {
                                        String categorieId = firstCategorie.getString("id");
                                        localRecord.setCatid(Integer.parseInt(categorieId));
                                    }
                                }
